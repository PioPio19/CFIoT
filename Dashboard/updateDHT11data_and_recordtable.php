<?php
  ...
  if (!empty($_POST)) {
    ...
    $sql = "UPDATE esp32_table_dht11_leds_update SET temperature = ?, humidity = ?, soil = ?, status_read_sensor_dht11 = ?, time = ?, date = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($temperature,$humidity,$soil,$status_read_sensor_dht11,$tm,$dt,$id));
    Database::disconnect();
    //........................................ 
    ...
    // This table is also used to store and record the state of the LEDs, the state of the LEDs is controlled from the "home.php" page. 
    // This table is operated with the "INSERT" command, so this table will contain many rows.
		$sql = "INSERT INTO esp32_table_dht11_leds_record (id,board,temperature,humidity,soil,status_read_sensor_dht11,LED_01,LED_02,time,date) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($id_key,$board,$temperature,$humidity,$soil,$status_read_sensor_dht11,$led_01,$led_02,$tm,$dt));
    //::::::::
